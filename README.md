...
- **Article skill** — `lipost article <url>`. Pipe an article URL through Claude Code, optionally generate an image with OpenAI, review the draft, and publish (or queue).
...
- For `--with-image`: `OPENAI_API_KEY` with access to OpenAI image generation. By default lipost uses `gpt-image-2`, `1024x1024`, `medium`, `jpeg`.
